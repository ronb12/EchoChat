...
export default function LoginModal() {
  const { closeLoginModal, showLoginModal, showNotification } = useUI();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (isSignUp) {
        // Sign up new user
        result = await authService.signUp(email, password, displayName || email.split('@')[0]);
      } else {
      ...
      if (result.success) {
        ...
        setError(result.error || (isSignUp ? 'Failed to create account' : 'Failed to sign in'));
      }
    } catch (err) {
      setError(err.message || 'An error occurred');
    } finally {
